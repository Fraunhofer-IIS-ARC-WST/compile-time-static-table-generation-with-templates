...
struct Helper<POLYNOM, TABLE_SIZE, D...> {
  static constexpr std::array<uint8_t, TABLE_SIZE> table = { D... };
};

constexpr std::array<uint8_t, TABLE_SIZE> crcTable = Helper<MY_POLYNOM>::table;

constexpr auto calcCRC(uint8_t len, const uint8_t* data) {
  uint8_t checksum = 0;
  while(len--) {
    checksum = crcTable[*data++ ^ checksum];
  }
  return checksum;
}
...
int main() {
  ...
  for(int i = 0; i < 256; ++i) {
    std::cout << static_cast<int>(crcTable[i]) << std::endl;
  }
}
